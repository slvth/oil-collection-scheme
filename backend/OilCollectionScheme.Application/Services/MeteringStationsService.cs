...
namespace OilCollectionScheme.Application.Services
{
    ...
    {
        private readonly IMeteringStationsRepository _meteringStationsService;
        ...
        {
            return await _meteringStationsService.GetAllBySchemeId(schemeId);
        }

        public async Task<int> CreateMeteringStation(MeteringStation meteringStation)
        {
            return await _meteringStationsService.Create(meteringStation);
        }

        public async Task<int> DeleteMeteringStation(int meteringStationId)
        {
            return await _meteringStationsService.Delete(meteringStationId);
        }
        public async Task<int> UpdateMeteringStation(MeteringStation meteringStation)
        {
            return await _meteringStationsService.Update(meteringStation);
        }

        public async Task<List<MeteringStationType>> GetMeteringStationTypes()
        {
            return await _meteringStationsService.GetTypes();
        }
